...
import { defineTool, renderToolsSdk, renderToolsSdkPy, RUN_CODE_NAME, } from '@deepseek-ai/dsh-tools';
import { Bm25Index } from "./bm25.js";
import { buildCatalog } from "./catalog.js";
import { ToolPolicyResolver, normalizeSettings } from "./settings.js";
import { MAX_RESULT_LIMIT, TOOL_SEARCH_NAME, TOOL_SDK_LANGUAGE_MARKERS, TOOL_SDK_SECTION_NAME, } from "./shared.js";
...
export class ToolSearchRuntime {
     ...
     */
    renderSdk(state, visible, original) {
        ...
        for (const tool of state.catalog) {
            ...
            schemas.push({
                ...
                parameters: cloneJson(definition?.parameters ?? tool.parameters),
                output: cloneJson(definition?.output?.schema ?? {}),
            });
        }
        return render(schemas);
    }
    resolveSdkRenderer(original) {
        const runtime = this.codeRuntime();
        if (runtime.language === 'typescript')
            return renderToolsSdk;
        if (runtime.language === 'python')
            return renderToolsSdkPy;
        if (original.includes(TOOL_SDK_LANGUAGE_MARKERS.python))
            return renderToolsSdkPy;
        if (original.includes(TOOL_SDK_LANGUAGE_MARKERS.typescript))
            return renderToolsSdk;
        throw new Error(`${TOOL_SEARCH_NAME}: cannot filter the generated tool SDK for code runtime language `
